...
const RegisterPortal = () => {
  ...
  const [form, setForm] = useState({ nama: "", email: "", password: "", dob: "", domisili: "", tiktok: "" });
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      playSound("click");
      if (err.code === "auth/email-already-in-use") alert("Email sudah terdaftar, silakan login");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#020617] via-black to-[#020617] flex items-center justify-center p-6 text-white">
      <motion.div initial={{ opacity: 0, y: 40, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.4 }}
        ...
        <div className="grid grid-cols-2 gap-2 mb-6">
          {[1, 2].map((g) => (
            <motion.button key={g} type="button" whileTap={{ scale: 0.9 }} whileHover={{ scale: 1.05 }}
              onClick={() => { playSound("click"); setGen(g); }}
              className={`p-3 rounded-xl text-xs font-bold transition ${gen === g ? "bg-blue-600 text-white shadow-lg" : "bg-gray-900 text-gray-400 hover:bg-gray-800"}`}>
              GEN {g}
            </motion.button>
          ))}
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-3">
          ...
          <input type="date" value={form.dob}
            onChange={(e) => { playSound("click"); setForm({ ...form, dob: e.target.value }); }}
          ...
          <select value={form.domisili}
            onChange={(e) => { playSound("click"); setForm({ ...form, domisili: e.target.value }); }}
            className="w-full p-3 bg-black/40 border border-gray-800 rounded-xl text-xs focus:ring-2 focus:ring-blue-500 outline-none text-gray-300">
          ...
          <motion.button type="submit" whileTap={{ scale: 0.95 }} whileHover={{ scale: 1.02 }} disabled={loading}
        ...
        <button type="button" onClick={() => { playSound("nav"); navigate("/login"); }}
