...
const Dashboard = () => {
  ...
  useEffect(() => {
    // 1. Ambil NASA APOD (Background process)
    getNasaAPOD()
      .then(data => setApod(data))
      .catch(err => console.log("NASA API Error:", err));
    ...
    if (saved) {
      ...
      // Jika data hilang, lempar ke register
      navigate("/register");
    }
  }, [navigate]);
  ...
  const handleLogout = () => {
    if (window.confirm("TERMINATE SESSION: Putus koneksi dari Satelit EAS?")) {
      localStorage.clear(); 
      window.location.href = "/register"; // Hard reset untuk bersihkan state
    }
  };

  // Mencegah "Flash of Unstyled Content" sebelum data ada
  if (isLoading) return (
    ...
    <div className="min-h-screen bg-[#00050d] text-white p-4 pb-24 font-mono animate-in fade-in duration-700">
      ...
      <div className="bg-gray-900/20 rounded-3xl overflow-hidden border border-blue-900/30 mb-6 backdrop-blur-sm">
        {apod ? (
          ...
          <div className="h-44 w-full bg-blue-950/10 animate-pulse flex items-center justify-center">
            <p className="text-[8px] tracking-widest text-blue-800 font-black uppercase">Syncing Satellite...</p>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2 bg-gradient-to-r from-blue-900/20 to-black p-5 rounded-3xl border border-blue-500/20 flex items-center justify-between mt-2 shadow-lg shadow-blue-900/10">
          <div>
            <h3 className="font-black text-[11px] tracking-widest text-white uppercase">
              Class: GEN {userData?.gen || "2"}
            </h3>
...
const MenuCard = ({ icon, title, desc, onClick }) => (
  <button 
    onClick={onClick}
    className="bg-gray-900/30 p-5 rounded-3xl border border-blue-900/20 hover:border-cyan-500/50 transition-all cursor-pointer group active:scale-95 text-left w-full"
  >
