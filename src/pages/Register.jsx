...
const RegisterPortal = () => {
  ...
  const handleRegister = async (e) => {
    ...
    if (parseInt(form.umur) < 10) {
      alert("AKSES DITOLAK: minimal 10 tahun.");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const aiCheck = await axios.post(
        "https://openrouter.ai/api/v1/chat/completions",
        {
          ...
          messages: [
            {
              role: "user",
              content: `Apakah nama "${form.nama}" adalah nama manusia wajar? jawab VALID atau INVALID`
            }
          ]
        },
        {
      ...
      if (!isValid) {
        alert("Nama tidak valid menurut AI.");
        setLoading(false);
        return;
      }
    ...
    alert("Registrasi berhasil. Generate ID Card...");
